...
[CustomPropertyDrawer(typeof(ReplaceValueDictionary))]
public class MyDictionaryDrawer3 : DictionaryDrawer<int, int> { }


[CustomPropertyDrawer(typeof(PlayerCharacterDictionary))]
public class MyDictionaryDrawer2 : DictionaryDrawer<string, int> { }
...
public abstract class DictionaryDrawer<TK, TV> : PropertyDrawer
{
    ...
    private void CheckInitialize(SerializedProperty property, GUIContent label)
    {
        if (_Dictionary == null)
        {
            ...
            _Dictionary = SafeValue(target, fieldInfo);
            if (_Dictionary == null)
            {
    ...
    private SerializableDictionary<TK, TV> SafeValue(object obj, FieldInfo info)
    {
        try
        {
            return fieldInfo.GetValue(obj) as SerializableDictionary<TK, TV>;
        }
        catch
        {
            var find = FindDictionary(obj, info);
            if (find != null) return find.One;
            
            throw new ArgumentException("No dictionary found and no child with the dictionary found!");
        }
    }

    private Tuple<SerializableDictionary<TK, TV>, object> FindDictionary(object current, FieldInfo info, List<object> stack = null)
    {
        if (stack == null)
        ...
        if (currentType.IsPrimitive || currentType.IsArray || current is string) return null;

        var fields = current.GetType().GetFields();

        foreach (var f in fields)
        {
            var obj = f.GetValue(current);
            if (f.Name == info.Name && f.FieldType == info.FieldType)
            {
                var t = new Tuple<SerializableDictionary<TK, TV>, object>();
                t.One = obj as SerializableDictionary<TK, TV>;
                t.Two = current;
                return t;
            }

            var result = FindDictionary(obj, info, stack);
            if (result == null)
                continue;
            return result;
        }
